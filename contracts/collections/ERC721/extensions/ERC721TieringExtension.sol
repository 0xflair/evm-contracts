    ...
    ReentrancyGuard
{
        ...
        onlyOwner
    {
        ...
        require(totalReserved <= maxSupply, "TIERING/MAX_SUPPLY_EXCEEDED");
    }
    ...
    function setTierMintingWindow(
        uint256 tierId,
        uint256 start,
        uint256 end
    ) external onlyOwner {
        tiers[tierId].start = start;
        tiers[tierId].end = end;
    }

    function setTierCurrency(uint256 tierId, address newValue)
        external
        onlyOwner
    {
        tiers[tierId].currency = newValue;
    }

    function setTierPrice(uint256 tierId, uint256 newValue) external onlyOwner {
        tiers[tierId].price = newValue;
    }

    function setTierMaxPerWallet(uint256 tierId, uint256 newValue)
        external
        onlyOwner
    {
        tiers[tierId].maxPerWallet = newValue;
    }

    function setTierMerkleRoot(uint256 tierId, bytes32 newRoot)
        external
        onlyOwner
    {
        tiers[tierId].merkleRoot = newRoot;
    }

    function setTierReserved(uint256 tierId, uint256 newValue)
        external
        onlyOwner
    {
        totalReserved -= tiers[tierId].reserved;
        tiers[tierId].reserved = newValue;
        totalReserved += newValue;
    ...
    function eligibleForTier(
        ...
        require(tiers[tierId].maxPerWallet > 0, "TIERING/NOT_EXISTS");
        require(block.timestamp >= tiers[tierId].start, "TIERING/NOT_STARTED");
        require(block.timestamp <= tiers[tierId].end, "TIERING/ALREADY_ENDED");
    ...
    function mintByTier(
        ...
        require(count > 0, "TIERING/MISSING_COUNT");
